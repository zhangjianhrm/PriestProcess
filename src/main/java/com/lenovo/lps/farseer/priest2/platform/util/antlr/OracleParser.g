/* java -Xmx512m org.antlr.Tool OracleParser.g */
...
@header{package com.lenovo.lps.farseer.priest2.platform.util.antlr;}
...
@init    {    int mode = 0;    }
    :    select_key
        ...
        group_by_clause?
        model_clause?
        -> {mode == 1}? ^(select_key from_clause ^(SELECT_OPTIONS distinct_key? unique_key? all_key?) ^(SELECT_LIST ASTERISK)
                into_clause? where_clause? hierarchical_query_clause? group_by_clause? model_clause?)
        -> ^(select_key from_clause ^(SELECT_OPTIONS distinct_key? unique_key? all_key?) ^(SELECT_LIST selected_element+)
                into_clause? where_clause? hierarchical_query_clause? group_by_clause? model_clause?)
    ;
...
from_clause
    :    from_key table_ref_list -> ^(FROM_CLAUSE table_ref_list)
    ;
...
join_clause
    :    query_partition_clause?
        join_qualifier1? join_qualifier2? JOIN
        table_ref_aux
        query_partition_clause?
    (    join_on_part
    |    join_using_part
    )*
        -> ^(JOIN_DEF ^(JOIN_QUALIFIER join_qualifier1? join_qualifier2?) table_ref_aux query_partition_clause* join_on_part* join_using_part*)
    ;
...
join_qualifier1:
    ...
    ;
join_qualifier2:
    INNER | outer_join_type
    ;

join_on_part
    :    on_key condition
        -> ^(JOIN_ON_PART ^(LOGIC_EXPR condition)) 
    ;
...
group_by_clause
    :    (group_key) => group_key by_key group_by_elements ((COMMA group_by_elements)=> COMMA group_by_elements)* having_clause?
         -> ^(GROUP_BY_CLAUSE ^(GROUP_BY_ELEMENT group_by_elements)+ having_clause?)
...
having_clause
    :    having_key condition
        -> ^(HAVING_CLAUSE ^(LOGIC_EXPR condition))
    ;
...
order_by_clause
    :    order_key siblings_key? by_key order_by_elements (COMMA order_by_elements)*
        -> ^(ORDER_BY_CLAUSE siblings_key? ^(ORDER_BY_ELEMENTS order_by_elements+))
    ;
...
update_statement
    :    update_key general_table_ref
		...
		-> ^(UPDATE_STATEMENT general_table_ref update_set_clause where_clause? static_returning_clause? error_logging_clause?)
    ;
...
insert_statement
    :    insert_key
    (    single_table_insert -> ^(INSERT_STATEMENT single_table_insert)
    |    multi_table_insert -> ^(INSERT_STATEMENT multi_table_insert)
    )
    ;
...
merge_statement
    :    merge_key into_key tableview_name table_alias?
        ...
        error_logging_clause?
        -> ^(MERGE_STATEMENT table_alias? tableview_name ^(using_key selected_tableview ^(LOGIC_EXPR condition))
...
lock_table_statement
    :    lock_key table_key 
        ...
        -> ^(LOCK_TABLE_STATEMENT lock_table_element+ lock_mode wait_nowait_part?)
    ;
...
additive_expression
    :  	special_function -> ^(SPECIAL_FUNC special_function)
	|	(multiply_expression ( ( PLUS_SIGN^ | MINUS_SIGN^ ) multiply_expression)*)
    ;

special_function
	:	sysdate_interval_key CHAR_STRING REGULAR_ID
	;
...
stantard_function_enabling_over
    :    {enablesOverClause(input.LT(1).getText())}?=> REGULAR_ID -> FUNCTION_ENABLING_OVER[$REGULAR_ID]
    ;

stantard_function_enabling_using
    :    {enablesUsingClause(input.LT(1).getText())}?=> REGULAR_ID -> FUNCTION_ENABLING_USING[$REGULAR_ID]
    ;

stantard_function_enabling_within_or_over
    :    {enablesWithinOrOverClause(input.LT(1).getText())}?=> REGULAR_ID -> FUNCTION_ENABLING_WITHIN_OR_OVER[$REGULAR_ID]
    ;

stantard_function_pedictions
    :    {isStandardPredictionFunction(input.LT(1).getText())}?=> REGULAR_ID -> PREDICTION_FUNCTION[$REGULAR_ID]
    ;
