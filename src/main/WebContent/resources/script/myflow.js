//处理破坏json串的"'",""","\t","\n","\r"字符
window.SelJoin = false,
	JoinStart = false;

function replaceAbnormalJsonStr(str){
	if (str && str !=""){
	//特殊字符转化
...
(function(b) {
	...
	a.rect = function(p, m) {
		var u = this, g = "rect" + a.util.nextId(), E = b.extend(true, {},
		...
		b([ t.node, f.node, n.node, e.node ]).bind(
				...
				function() {
					...
					var _isJoin = $(this).attr('href') != undefined && $(this).attr('href').indexOf('gateway_exclusive.png') > 0;
					...
					case "path":
						...
						if (r && r.getId() != g
							...
							//工具栏线条结点失去焦点,选择结点获取焦点
							//b('#path').removeClass('selected');
							//b('#pointer').addClass('selected');
						}
		...
		//处理右键点击,保存当前结束属性
		b([ t.node, f.node, n.node, e.node ]).mousedown(function(e){
			//e.which :1左键;2中键;3右键
			if (e.which == 3){
				$(window).data("currProps",E.props);
			}
		});		
		var j = function(o, r) {
			if (!a.config.editable) {
...
//				}).show();
				f.attr({
					x : F + E.img.width + (G - E.img.width)/2 + 3,
					y : r + 22
				}).show();
				e.attr({
					x : F + E.img.width / 2,
			...
			//iframe动态增长
			var iframeObj = (document.getElementById)? window.parent.document.getElementById('pIframe'):window.parent.document.all['pIframe'];
			//iframeObj.contentDocument为w3c标准
			//window.parent.document.frames['pIframe'].document为IE特色
            var iFrameDoc = (iframeObj.contentDocument)?iframeObj.contentDocument:window.parent.document.frames['pIframe'].document;
            
            //iframe内容高度底部Y值
			var bottomY = iFrameDoc.body.scrollHeight;

			//如果当前方框Y值与iframe底端距离不足100,iframe向下延伸100
			if(bottomY - r < 100){
				b(iframeObj).css('height','+=100');
				b('#myflow').find('svg').css('height','+=100');
			}
		...
		this.toJson = function() {
			var r = '{type:"' + E.type + '",text:{text:"' + replaceAbnormalJsonStr(f.attr("text"))
			...
			for ( var o in E.props) {
				r += o + ':{value:"' + replaceAbnormalJsonStr(E.props[o].value) + '"},'
			}
	...
	a.path = function(q, n, u, e) {
		var v = this, z = n, B = b.extend(true, {}, a.config.path), i, t, f, h = B.textPos, y, w, k = u, s = e, g = "path"
				+ a.util.nextId(), x;
		function p(G, H, D, L) {
		...
		B.text.text = JoinStart ? '分支条件' : B.text.text;
...
//		f['isJoin'] = JoinStart;
		...
		this.toJson = function() {
			var r = '{from:"' + k.getId() + '",to:"' + s.getId() + '", dots:'
					+ x.toJson() + ',text:{text:"' + replaceAbnormalJsonStr(f.attr("text"))
					+ '"},textPos:{x:' + Math.round(h.x) + ",y:"
					+ Math.round(h.y) + "}, props:{";
			for ( var o in B.props) {
				r += o + ':{value:"' + replaceAbnormalJsonStr(B.props[o].value) + '"},'
			}
			if (r.substring(r.length - 1, r.length) == ",") {
				r = r.substring(0, r.length - 1)
			}
	...
	a.props = function(h, f) {
		...
		var d = function(n, m, o) {
			// 连续点击不刷新(有一种情况除外：HDFS操作，用户选择操作，动态生成属性，特征：属性包括hdfsOp)
			if (i && i.getId() == o.getId() && !m.hdfsOp) {
				return;
			}
			else{
				i = o;
			...
			for ( var l in m) {
				var _hidden = '';
				var _alermPersonClass = '';
				if(l == 'appName' || l == 'lateStartTime' || l == 'validate' || l == 'priority' || l == 'appVariables'){
				   _hidden = 'isHide';
				   if(l == 'alermPerson'){
					   _alermPersonClass = ' alermPerson';
				   }
			    }
				...
				if(l == 'alermPerson'){
				   ...
				   if($('#palermMethod select').val() != 'S'){
					   _hidden = 'isHide';
				   }
			    }
				
				if(l == 'overWrite' && ($('#ptransferMode select').val() == 'H2M' || $('#ptransferMode select').val() == 'H2O')){
					_hidden = 'isHide';
				}
				
				if(l == 'out' && ['upload', 'download', 'remove', 'remove_quiet', 'copy', 'move'].indexOf($('#phdfsOp select').val()) > -1){
					_hidden = 'isHide';
				}
				
				e.append("<tr class='"+_hidden+ _alermPersonClass + "'><th style='text-align: right;' >" + m[l].label + '</th><td style="width:150px;"><div id="p' + l
						+ '" class="editor"></div></td></tr>');
				if (m[l].editor) {
					m[l].editor().init(m, l, "p" + l, o, g);
				}
			}
			$('#myflow_props_handle span').html('属性');
		};
		b(g).bind("showprops", d);
	};
	a.editors = {
		textEditor : function(propName) {
			if(propName){
				$('#MyflowProps').data('type', propName);
			}else{
				$('#MyflowProps').data('type', 'prop');
			}
			var d, e, c, g, f;
			this.init = function(i, h, m, l, j) {
				...
				if(g.text() == '分支条件' || i[e]['isJoinStart']){
					setTimeout(function(){
						$('#myflow_props_handle span').html('分支条件');
					}, 200)
					$('#myflow_props').find('th').eq(0).html('表达式');
					
					i[e]['isJoinStart'] = true;
					var _val = g.text() == '分支条件' ? '' : g.text();
					
					b('<input style="width:100%;"/>').attr('placeholder', '表达式').val(_val).change(
							function() {
								var _val = b(this).val();
								_val = _val == '' ? '分支条件' : _val;
								i[e].value = _val;
								b(f).trigger("textchange", [_val, g])
							}).appendTo("#" + c);
					$('#MyflowProps').data('type', 'joinLine');
					
				}else{
					$('#myflow_props_handle span').html('属性');
					$('#myflow_props').find('th').eq(0).html('显示');
				...
				if(g.getId().substring(0,4)=="path"){
					var tmpb = g.text();
					
//					if(tmpb.indexOf("(" + $.i18n.prop('weekDepend') + ")") != -1){
//						b('<input  style="width:100%;"/>').val(g.text().replace("(" + $.i18n.prop('weekDepend') + ")","")).appendTo("#" + c);
//					}else{
//						b('<input  style="width:100%;"/>').val(g.text()+"(" + $.i18n.prop('weekDepend') + ")").appendTo("#" + c);
//					}
				}
				b("#" + c).data("editor", this);
				
				if(propName && propName == 'notify'){
					setTimeout(function(){
	...
	a.init = function(x, r) {
		...
		b(document).keydown(function(i) {
			...
			//只有在画板中点击"delete"时,才删除
			if (i.keyCode == 46 && (i.target.nodeName == 'BODY' || i.target.nodeName == 'svg')) {
		...
		b(document).click(function() {
			SelJoin = false;
			// 用户点击画板时触发
			if (b(window).data("popupClick") == null){
			...
			// 用户在弹出框在输入完数据,或者选择cancel放弃输入时,会触发画板的click事件,通过$(window).data("popupClick")做标记
			else{
				// 事件已经触发,清除标记
				b(window).data("popupClick",null);
			}
		});
		...
		var f = function(i, k, j) {
			...
			for ( var c in g) {
				if (g[c]) {
					var from = g[c].from().getId();
					var to = g[c].to().getId();
					
					if ((from == k.getId() && to == j.getId()) || (from == j.getId() && to ==k.getId())){
						alert($.i18n.prop('lineExist'));
						return false;
					}
				}
			}	
		...
		if (a.config.editable) {
			...
			b("#myflow_tools .selectable").click(function() {
				b(".tool_selected").removeClass("tool_selected");
				b(this).addClass("tool_selected");
			...
			b("#myflow_save").click(
					function() {
						...
						for ( var c in a.config.props.props) {
							i += c + ':{value:"'
									+ replaceAbnormalJsonStr(a.config.props.props[c].value) + '"},'
						}
						if (i.substring(i.length - 1, i.length) == ",") {
							i = i.substring(0, i.length - 1)
						}
						...
						var encoded = JSON.stringify(eval('(' + i + ')'));
						a.config.tools.save.onclick(encoded)
					});
			new a.props({}, y)
		}
		//恢复图形
		if (r.restore) {
			...
			if (B.states) {
				for ( var s in B.states) {
					var rect_props =b.extend(true, {},a.config.tools.states[B.states[s].type],B.states[s]);
					
					//数据迁移默认没有数据源，需要手动添加
					if (B.states[s].type == 'transfer'){
						var transferMode=B.states[s].props.transferMode.value;
						
						//只有MySQL/Oracle <===> Hive迁移时需要选择数据源
						if (transferMode =='H2M' || transferMode =='M2H' ){
							rect_props =b.extend(true, {},rect_props,{props : {rdbmsDB : { name : 'rdbmsDB',	label : $.i18n.prop('mysql'),value : '', editor : function() {	
						...
						}else if (transferMode =='H2O' ||transferMode =='O2H') {
							rect_props =b.extend(true, {},rect_props,{props : {rdbmsDB : { name : 'rdbmsDB',	label : $.i18n.prop('oracle'),value : '', editor : function() {	
								var appId = $.myflow.config.props.props.appName.value;var url =  '../config/getAppDbNames.do?appId=' + appId + '&dbType=' + 2;	return new $.myflow.editors.selectEditor(url);}}}});
						}
					}else if(B.states[s].type == 'hdfs'){
                        var op = B.states[s].props.hdfsOp.value ;
                        if ( op == 'upload' ){
                            var val = B.states[s].props.srcPath.value ||'';
                            rect_props = b.extend(true,{},rect_props,{props:{srcPath:{name : 'srcPath',label : $.i18n.prop('localFile'), value : val ,editor : function() {return new $.myflow.editors.fileUploadEditor();} }}});
                        }
					}else if(B.states[s].type == 'dep'){
						var depType = B.states[s].props.depType.value ;
						if (depType == 'process') {
							rect_props = b.extend(true,{},rect_props,{props:{target:{name : 'target',label : $.i18n.prop('process') + ' ID', value : '' ,editor : function() {return new $.myflow.editors.inputEditor();} }}});
						} else if ( depType == 'hdfs_dir' ){
							rect_props = b.extend(true,{},rect_props,{props:{target:{name : 'target',label : $.i18n.prop('filePath'), value : '' ,editor : function() {return new $.myflow.editors.inputEditor();} }}});
						} else if ( depType == 'hdfs_size' ) {
							rect_props = b.extend(true,{},rect_props,{props:{target:{name : 'target',label : $.i18n.prop('filePath'), 
							...
							var depDbType=B.states[s].props.depDbType.value;
							if (depDbType =='spark') {
								rect_props = b.extend(true,{},rect_props,{props:{depDbType:{name : 'depDbType',label : $.i18n.prop('depDbType'), 
			...
			for ( var u = 0; u < A.rects.length; u++) {
				...
				for ( var t = 0; t < A.rects[u].paths.length; t++) {
					if (z[A.rects[u].name].paths[A.rects[u].paths[t]]) {
						z[A.rects[u].name].paths[A.rects[u].paths[t]]
								.attr(A.pathAttr)
					}
				}
			}
			for ( var u = 0; u < l.rects.length; u++) {
				...
				for ( var t = 0; t < l.rects[u].paths.length; t++) {
					if (z[l.rects[u].name].paths[l.rects[u].paths[t]]) {
						z[l.rects[u].name].paths[l.rects[u].paths[t]]
								.attr(l.pathAttr)
					}
				}
			}
