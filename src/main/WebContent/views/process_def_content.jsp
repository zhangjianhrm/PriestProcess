			...
			loadComplete: function(xhr){
				...
				for(var i = 0;i<ids.length;i++){
				    ...
				    if (processState == 'D'){
				    	...
				    	opeAnchor = "<a href='javascript:;' class='glyphicon glyphicon-start' alt='<%=bundleJsp.getString("processEnable")%>' title='<%=bundleJsp.getString("processEnable")%>' data-toggle='tooltip' data-placement='top'  onclick=\"updateState('" + processId + "','N');\" ></a>" ;		    	
				    }
				    //正常状态
					...
					if ($('#currentUserHidden').val() == xhr.rows[i]['createdBy']){
				    	// opeAnchor +="&nbsp;&nbsp;<img src='../img/download.jpg'  alt='导出流程' title='导出流程' onclick=\"dataoutput('" + processId + "');\" />";
				    	opeAnchor +="&nbsp;&nbsp;<a href='javascript:;' class='glyphicon glyphicon-download' alt='<%=bundleJsp.getString("exportProcess")%>' title='<%=bundleJsp.getString("exportProcess")%>' data-toggle='tooltip' data-placement='top' onclick=\"dataoutput('" + processId + "');\" ></a>";
					}
